/**
 * productoForm.js — Formulario unificado Agregar / Editar.
...
document.addEventListener('DOMContentLoaded', function () {
        ...
        form.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    btn.disabled = false;
                    btnLabel.textContent = MODO_EDITAR ? 'Actualizar Producto' : 'Guardar Producto';
    ...
    document.querySelectorAll('.img-delete-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            showConfirmImg(async () => {
                ...
                try {
                    const res = await fetch(`${BASE_URL}/dashboard/productos/imagen/borrar`, {
                        method: 'POST',
                        body: fd,
                        headers: { 'Accept': 'application/json' },
                    });
    ...
    document.querySelectorAll('.img-replace-input').forEach(input => {
        input.addEventListener('change', async function () {
            ...
            try {
                const res = await fetch(`${BASE_URL}/dashboard/productos/imagen/reemplazar`, {
                    method: 'POST',
                    body: fd,
                    headers: { 'Accept': 'application/json' },
                });
