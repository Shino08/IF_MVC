...
<div class="bg-gray-50 min-h-screen py-10">
    <div class="max-w-6xl mx-auto px-4">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600 mt-2">Pedido #PED-<?= date('Y', strtotime($carrito['fecha_solicitud'])) ?>-<?= str_pad((string)$carrito['id'], 4, '0', STR_PAD_LEFT) ?></p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="space-y-4">
                            <div class="space-y-2 text-sm text-gray-600">
                                <div class="flex justify-between">
                                    <span>Subtotal:</span>
                                    <span class="font-medium text-gray-900">$<?= number_format($subtotal ?? 0, 2) ?></span>
                                </div>
                                <?php if (isset($carrito['aplica_iva']) && $carrito['aplica_iva'] == 1): ?>
                                <div class="flex justify-between">
                                    <span>IVA (<?= number_format((float)($carrito['tasa_iva'] ?? 16), 0) ?>%):</span>
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <span class="font-medium text-gray-900">$<?= number_format($carrito['costo_envio'], 2) ?></span>
                                </div>
                                <?php endif; ?>
                            </div>
                            <div class="pt-3 border-t border-gray-200">
                                <p class="text-xs text-gray-500 font-semibold uppercase">Monto a Pagar</p>
                                <p class="text-2xl font-bold text-red-600">$<?= number_format($totalFinal ?? 0, 2) ?></p>
                            </div>
                            <?php if (!empty($carrito['tasabcv'])): ?>
                            <div class="pt-4 border-t border-gray-200">
                                <p class="text-xs text-gray-500 font-semibold uppercase">Equivalente BCV</p>
                                <p class="text-lg font-bold text-gray-900">Bs. <?= number_format($totalFinal * $carrito['tasabcv'], 2, ',', '.') ?></p>
                                <p class="text-xs text-gray-400 mt-1">Tasa: Bs. <?= number_format($carrito['tasabcv'], 4, ',', '.') ?></p>
                            </div>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-xl p-5 text-sm text-blue-800 shadow-sm">
                    ...
                    <ul class="list-disc list-inside space-y-2">
                        ...
                        <li><strong>Transferencia:</strong><br>Banesco (0134)<br>Cuenta Corriente<br>0134-0000-00-0000000000<br>A nombre de: InstalFuego C.A.<br>RIF: J-12345678-9</li>
                    </ul>
                    <div class="mt-4 pt-4 border-t border-blue-200">
                        <p class="font-semibold mb-2">¿Deseas pagar en divisas o efectivo?</p>
                        <a href="https://wa.me/584121234567?text=Hola,%20quiero%20coordinar%20el%20pago%20en%20divisas%20del%20pedido%20PED-<?= date('Y', strtotime($carrito['fecha_solicitud'])) ?>-<?= str_pad((string)$carrito['id'], 4, '0', STR_PAD_LEFT) ?>" target="_blank" class="inline-flex items-center justify-center w-full px-4 py-2.5 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 transition-colors shadow-sm">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <form action="<?= $base_url ?? '' ?>/pedido/pagar/<?= $carrito['id'] ?>" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
                            ...
                            <div>
                                ...
                                <?php 
                                $montoBs = isset($carrito['tasabcv']) ? round($totalFinal * $carrito['tasabcv'], 2) : 0; 
                                ?>
                                <input type="number" step="0.01" name="monto" class="input-elegant bg-gray-50 text-gray-500 cursor-not-allowed" value="<?= $montoBs ?>" readonly required>
                                <p class="text-xs text-gray-500 mt-1">El monto se calcula automáticamente según la tasa BCV actual.</p>
                            </div>
                        ...
                        <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
                            <a href="<?= $base_url ?? '' ?>/mis-pedidos/<?= $carrito['id'] ?>" class="px-6 py-2.5 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors">Cancelar</a>
