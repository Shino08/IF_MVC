...
$imgDir       = ($base_url ?? '') . '/img/productos/';
$formAction   = $modoEditar
    ? ($base_url ?? '') . '/dashboard/productos/actualizar/' . $productoId
    : ($base_url ?? '') . '/dashboard/productos/store';
$tituloHeader = $modoEditar ? 'Editar Producto' : 'Agregar Nuevo Producto';
$subHeader    = $modoEditar ? 'Modifica los datos del producto' : 'Introduce los detalles técnicos y de inventario';
$btnLabel     = $modoEditar ? 'Actualizar Producto' : 'Guardar Producto';
        ...
        <header class="bg-white border-b border-gray-200 px-8 py-5 flex-shrink-0">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-sm"><?= $subHeader ?></p>
                </div>
            </div>
        </header>
            ...
            <div class="grid grid-cols-2 xl:grid-cols-4 gap-8">
                ...
                <div class="xl:col-span-3">
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        ...
                        <form action="<?= $formAction ?>" method="POST" enctype="multipart/form-data" id="productoForm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <input type="text" id="nombre" name="nombre" placeholder="Ej: Extintor de PQS 10 Lbs"
                                ...
                                <div>
                                    ...
                                    <input type="text" id="sku" name="sku" placeholder="Ej: EXT-PQS-10"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <select id="categoria_id" name="categoria_id"
                                            class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500 bg-white" required>
                                        <option value="">Seleccione una categoría</option>
                                        <?php foreach ($categorias as $cat): ?>
                                            <option value="<?= $cat['id'] ?>"
                                                <?= ($producto['categoria_id'] ?? 0) == $cat['id'] ? 'selected' : '' ?>>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                <div>
                                    ...
                                    <input type="text" id="marca" name="marca" placeholder="Ej: Kidde"
                                ...
                                <div>
                                    ...
                                    <input type="text" id="modelo" name="modelo" placeholder="Ej: Pro 10"
                                           value="<?= htmlspecialchars($producto['modelo'] ?? '') ?>"
                                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                                </div>
                                <div>
                                    <label for="existencia" class="block text-sm font-semibold text-gray-900 mb-2">Stock</label>
                                    <input type="number" id="existencia" name="existencia" min="0"
                                           value="<?= htmlspecialchars((string)($producto['existencia'] ?? 0)) ?>"
                                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <textarea id="descripcion" name="descripcion" rows="4"
                                          placeholder="Especificaciones, usos recomendados, componentes..."
                                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 resize-none"><?= htmlspecialchars($producto['descripcion'] ?? '') ?></textarea>
                            </div>

                            <!-- ── IMÁGENES EXISTENTES (modo editar) ───────────────────── -->
                            <?php if ($modoEditar): ?>
                            <div class="mb-6" id="imagenes-existentes-section">
                                <h3 class="text-sm font-semibold text-gray-900 mb-3">Imágenes actuales</h3>
                                <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3" id="galeria-existente">

                                    <!-- IMAGEN PRINCIPAL -->
                                    <?php if (!empty($producto['imagen_principal'])): ?>
                                    <div class="relative group" id="img-principal-wrapper">
                                        <div class="aspect-square bg-white border-2 border-red-400 rounded-xl overflow-hidden">
                                            <img src="<?= $imgDir . htmlspecialchars($producto['imagen_principal']) ?>"
                                                 alt="Principal" class="w-full h-full object-contain p-1">
                                        </div>
                                        <span class="absolute top-1 left-1 bg-red-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded shadow">Principal</span>
                                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex flex-col items-center justify-center gap-2">
                                            <label class="cursor-pointer bg-white text-gray-800 text-xs font-bold px-2 py-1 rounded-lg hover:bg-gray-100 transition">
                                                ...
                                                <input type="file" accept="image/*" class="hidden img-replace-input"
                                                       data-tipo="principal" data-producto-id="<?= $productoId ?>" data-sku="<?= htmlspecialchars($producto['sku']) ?>">
                                            </label>
                                            <button type="button" class="img-delete-btn bg-red-600 text-white text-xs font-bold px-2 py-1 rounded-lg hover:bg-red-700 transition"
                                                    data-tipo="principal" data-producto-id="<?= $productoId ?>">
                                                Borrar
                                            </button>
                                        </div>
                                    </div>
                                        ...
                                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex flex-col items-center justify-center gap-2">
                                            <label class="cursor-pointer bg-white text-gray-800 text-xs font-bold px-2 py-1 rounded-lg hover:bg-gray-100 transition">
                                                ...
                                                <input type="file" accept="image/*" class="hidden img-replace-input"
                                                       ...
                                                       data-producto-id="<?= $productoId ?>" data-sku="<?= htmlspecialchars($producto['sku']) ?>">
                                            </label>
                                            <button type="button" class="img-delete-btn bg-red-600 text-white text-xs font-bold px-2 py-1 rounded-lg hover:bg-red-700 transition"
                                                    data-tipo="galeria" data-image-id="<?= $img['id'] ?>" data-producto-id="<?= $productoId ?>">
                                                Borrar
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-semibold text-gray-900 mb-2">
                                    <?= $modoEditar ? 'Agregar más imágenes' : 'Imágenes del Producto' ?>
                ...
                <div class="xl:col-span-1">
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-8">
                        ...
                        <div id="preview-section" class="mb-6">
                            <div id="image-placeholder" class="border-2 border-dashed border-gray-200 rounded-2xl aspect-square flex flex-col items-center justify-center p-4 text-center <?= ($modoEditar && !empty($producto['imagen_principal'])) ? 'hidden' : '' ?>">
                            ...
                            <?php if ($modoEditar && !empty($producto['imagen_principal'])): ?>
                            <div id="preview-principal-edit" class="aspect-square bg-white border border-gray-200 rounded-2xl overflow-hidden">
                                <img src="<?= $imgDir . htmlspecialchars($producto['imagen_principal']) ?>"
...
<!-- ── MODAL NOTIFICACIÓN (error / éxito) ──────────────────────── -->
<div id="notif-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
        <div class="flex items-center justify-center w-12 h-12 rounded-full mx-auto mb-4" id="notif-icon-wrap">
            <!-- icono dinámico -->
        </div>
...
<script src="<?= $base_url ?? '' ?>/js/productoForm.js"></script>
</body>
</html>